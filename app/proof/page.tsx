  ...
  return (
    <div className="min-h-screen bg-white text-ink">
      ...
      <main className="mx-auto max-w-[900px] px-6 max-[700px]:px-4 py-[44px]">
        ...
        {/* Security self-assessment · ROUND 4 — audited our own newest code */}
        <div className="mt-14 pt-2 border-t border-hair">
          <div className="text-[11px] uppercase tracking-[0.16em] font-semibold text-peri-deep font-atx-display mt-8">
            {SECURITY_AUDIT_R4.eyebrow}
          </div>
          <h2 className="font-atx-display font-bold text-[clamp(1.7rem,4.4vw,2.5rem)] leading-[1.06] tracking-[-0.03em] mt-3">
            {SECURITY_AUDIT_R4.title}<br /><span className="text-gradient-accent">{SECURITY_AUDIT_R4.titleAccent}</span>
          </h2>
          <p className="text-ink-mid text-[clamp(0.98rem,2vw,1.12rem)] leading-[1.5] max-w-[62ch] mt-4">{SECURITY_AUDIT_R4.blurb}</p>

          <div className="grid grid-cols-4 max-[720px]:grid-cols-2 gap-3 mt-6">
            {SECURITY_AUDIT_R4.stats.map((m) => (
              <div key={m.label} className="rounded-[16px] border border-hair bg-white shadow-card p-4">
                <div className="font-atx-display font-bold text-[1.7rem] tracking-[-0.02em] tabular-nums">
                  {m.value}<span className="text-peri-deep text-[1.1rem]"> {m.sub}</span>
                </div>
          ...
          <p className="text-[12px] text-ink-soft mt-4 max-w-[64ch]">{SECURITY_AUDIT_R4.note}</p>
        </div>

        {/* Security self-assessment · ROUND 5 — firm-grade (Hacken/CertiK SOP) */}
        <div className="mt-14 pt-2 border-t border-hair">
          <div className="text-[11px] uppercase tracking-[0.16em] font-semibold text-peri-deep font-atx-display mt-8">
            {SECURITY_AUDIT_R5.eyebrow}
          </div>
          <h2 className="font-atx-display font-bold text-[clamp(1.7rem,4.4vw,2.5rem)] leading-[1.06] tracking-[-0.03em] mt-3">
            {SECURITY_AUDIT_R5.title}<br /><span className="text-gradient-accent">{SECURITY_AUDIT_R5.titleAccent}</span>
          </h2>
          <p className="text-ink-mid text-[clamp(0.98rem,2vw,1.12rem)] leading-[1.5] max-w-[62ch] mt-4">{SECURITY_AUDIT_R5.blurb}</p>

          <div className="grid grid-cols-4 max-[720px]:grid-cols-2 gap-3 mt-6">
            {SECURITY_AUDIT_R5.stats.map((m) => (
              <div key={m.label} className="rounded-[16px] border border-hair bg-white shadow-card p-4">
                <div className="font-atx-display font-bold text-[1.7rem] tracking-[-0.02em] tabular-nums">
                  {m.value}<span className="text-peri-deep text-[1.1rem]"> {m.sub}</span>
                </div>
