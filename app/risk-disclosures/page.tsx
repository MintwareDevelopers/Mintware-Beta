  ...
  return (
    <div className="min-h-screen font-atx-display bg-white text-ink overflow-x-clip">
      ...
      <div className={wrap}>
        ...
        <Risk t="Stablecoin issuer controls (USDG)">The LP Gateway's quote asset on Robinhood Chain is USDG, a stablecoin issued by Paxos. Like most regulated stablecoins, USDG's contract gives its issuer the power to pause all transfers, to freeze the balance of any address, and to wipe a frozen balance — for example under a legal or sanctions order. Those powers sit with the issuer, not with Mintware, and we cannot override, delay, or reverse them. If the issuer freezes or wipes an address that holds gateway funds at rest (the yield source the idle leg sits in, the Uniswap pool, or a gateway contract), the affected value could become inaccessible or be permanently lost. This is the same class of risk every USDG (or USDC) holder carries; we mention it here because it applies to funds routed through the gateway exactly as it applies to funds held directly.</Risk>
        <Risk t="Third-party paired-token controls">Each gateway pool pairs USDG with a third-party token that Mintware does not issue or control. Some tokens carry administrative controls of their own — an owner who can pause transfers, blacklist or freeze addresses, mint new supply, or upgrade the contract's code. If such a control is used against the pool or a gateway address, the deployed liquidity leg of a withdrawal may be unable to execute until the restriction is lifted. The gateway is written so that the idle leg (the portion held in the yield source) still pays out and your claim on the blocked leg is retained as shares rather than lost, but it cannot make a paused or frozen token move. Our curation policy is to avoid pools whose paired token has such controls; that policy is a screening heuristic and a human judgement, not a guarantee, and a token's controls can change after listing.</Risk>
