...
export const POST = createHandler(async (_req, ctx) => {
  ...
  // Explicit nonce management: public RPCs lag `getTransactionCount(pending)`, so firing a rapid
  // sequence of server-wallet txs can reuse a nonce and silently replace an earlier tx (that dropped
  // the mint on the first run — approve landed, mint didn't). Pin the nonce once and increment
  // locally; assert each receipt succeeded at its own step so any revert surfaces precisely.
  let nonce = await publicClient.getTransactionCount({ address: account.address, blockTag: 'pending' })
  const confirm = async (hash: `0x${string}`, step: string) => {
    const r = await publicClient.waitForTransactionReceipt({ hash })
    if (r.status !== 'success') throw new Error(`${step} reverted (tx ${hash})`)
    return r
  }

  try {
    // 1. Deploy a mintable mock token.
    txs.tokenDeployTx = await walletClient.deployContract({
      abi: MOCK_ERC20_ABI, bytecode: MOCK_ERC20_BYTECODE, account, chain: baseSepolia,
      args: ['MW Smoke', 'MWS', 18], nonce: nonce++,
    })
    const tokenRcpt = await confirm(txs.tokenDeployTx as `0x${string}`, 'token-deploy')
    const token = tokenRcpt.contractAddress
    if (!token) return ctx.json({ ok: false, step: 'token-deploy', txs, error: 'no token address' }, 500)

    // 2. Mint to the Privy wallet.
    txs.mintTx = await walletClient.writeContract({
      address: token, abi: MOCK_ERC20_ABI, functionName: 'mint', args: [account.address, MINT], account, chain: baseSepolia, nonce: nonce++,
    })
    await confirm(txs.mintTx as `0x${string}`, 'mint')
    ...
    txs.registerTx = await walletClient.writeContract({
      ...
      args: [campaignId, token, account.address], account, chain: baseSepolia, nonce: nonce++,
    })
    await confirm(txs.registerTx as `0x${string}`, 'register')

    // 4. Approve + deposit into the campaign pool.
    txs.approveTx = await walletClient.writeContract({
      address: token, abi: MOCK_ERC20_ABI, functionName: 'approve', args: [distributor, DEPOSIT], account, chain: baseSepolia, nonce: nonce++,
    })
    await confirm(txs.approveTx as `0x${string}`, 'approve')
    txs.depositTx = await walletClient.writeContract({
      address: distributor, abi: CAMPAIGN_DISTRIBUTOR_ABI, functionName: 'depositCampaign', args: [campaignId, token, DEPOSIT], account, chain: baseSepolia, nonce: nonce++,
    })
    await confirm(txs.depositTx as `0x${string}`, 'deposit')
    ...
    return ctx.json({ ok: false, step: 'round', txs, error: err instanceof Error ? err.message : String(err) }, 500)
  }
}, { auth: 'bearer-token' })
