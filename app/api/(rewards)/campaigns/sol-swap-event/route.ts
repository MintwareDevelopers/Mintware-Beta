...
export const POST = createHandler(async (req, ctx) => {
  if (!solanaEnabled) {
    return ctx.json({ credited: false, skip_reason: 'solana_swaps_paused' }, 410)
  }
  ...
  const verification = await verifySolanaTx(body.tx_signature, body.wallet, SOLANA_SERVER_RPC_URL)
...
export const GET = createHandler(async (_req, ctx) => {
  if (!solanaEnabled) {
    return ctx.json({ error: 'solana_swaps_paused' }, 410)
  }
