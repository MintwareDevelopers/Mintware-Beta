...
//
// Solana claiming is intentionally paused on the live product surface.
// The deeper implementation remains in-repo, but this component stays EVM-only
// so paused Solana codepaths cannot affect production claiming.
// =============================================================================
...
function RewardRow({ reward, wallet, onClaimed }: RewardRowProps) {
  ...
  useEffect(() => {
    if (isTxSuccess && txHash && reward.distribution_id) {
      ...
      // The server verifies the actual distributor claim transaction before
      // updating off-chain claim state.
...
export function ClaimCard({ wallet }: ClaimCardProps) {
  ...
  const currentChainId = useChainId()
  const publicClient = usePublicClient()
  const { switchChain, isPending: isBatchSwitching } = useSwitchChain()
                  ...
                  <div className="flex flex-col items-end gap-[6px]">
                    ...
                    {isBatchWrongChain ? (
                      <button
                        ...
                        title={`Switch to ${batchGroupChainName ?? 'the correct chain'} before claiming`}
                      >
                        {isBatchSwitching ? 'Switching network…' : `Switch to ${batchGroupChainName ?? 'network'}`}
                      </button>
                    ) : (
                      <button
