...
4. **Gateway custodies NO funds (chosen design — trust win):** `burnForPayment(user, sharesBurned, receiver)` sends USDC **straight from the vault to the card rail**. The Gateway only authorizes — it can never hold or lose user money. (Do NOT route USDC through the Gateway.)
5. **`assets` vs `assetsRedeemed`:** `previewWithdraw(assets)` rounds shares UP → `burnForPayment` redeems `redeemed = floor(shares × NAV) ≥ assets`; Gateway asserts `redeemed ≥ assets`. The vault sends exactly `redeemed` to the receiver; the `redeemed − assets` over-transfer is bounded by ~1 share-unit of NAV (sub-cent) and is acceptable rail dust. The user's shares burn exactly matches what left the vault ⇒ solvency + NAV are preserved (verify in `invariant_settlement_conserves`).
