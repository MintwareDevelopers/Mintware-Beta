...
///                                  (verified + smoke-tested on-chain 2026-08-18). XyloVault is a non-standard
///                                  4626 (10bps withdrawFee + 10% perfFee; convertToAssets/maxWithdraw
///                                  over-report, withdraw() reverts near full — redeem() is the exit) — but
///                                  MintwareERC4626YieldAdapter is now FEE-AWARE (previewRedeem NAV + redeem
///                                  exit, 5 regression tests), so setting this is a clean drop-in. Gated only
///                                  on external audit. See config/arc.ts + docs/…/session-handoff-arc.md #1.
