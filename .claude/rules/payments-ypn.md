...
|---|---|---|
| **edge-auth** (axum) | Sub-150 ms authorization: decide → reserve a hold off cached **NAV** with a VaR haircut → sign. Fail-closed bearer. | `POST /authorize` · `GET /holds/:id` · `GET /available/:user` (read-only live spendable = NAV − holds − cap − liquidity) · `POST /webhooks/rain` |
...
Tests: `cd services/edge-auth && cargo test` (82) · `cd services/relayer && cargo test` (23); both
