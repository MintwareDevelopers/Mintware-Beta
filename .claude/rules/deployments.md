...
| `LP_GATEWAY_USDG` | Server-only | Quote-asset address **registry** matches a candidate PM's on-chain `quoteAsset()` against — must equal whatever the DEPLOYED rig actually uses (today the testnet mock tUSDG, see `deployments.json`). **RH mainnet (4663) real USDG = `0x5fc5360D0400a0Fd4f2af552ADD042D716F1d168`** — Paxos-NATIVE UUPS proxy, verified 2026-09-07 against Paxos' docs + on-chain (re-audit M-07). Issuer can freeze **and wipe** balances (asset-protection role) — bounded exposure + disclosure. ⚠ **2026-09-09: this is registry's var, NOT discovery's any more** — see `LP_GATEWAY_DISCOVER_USDG` below; they diverged when this was repointed to the rig's tUSDG on 2026-09-08, which silently emptied the Discover feed. |
| `LP_GATEWAY_DISCOVER_USDG` | Server-only | **NEW (2026-09-09 fix).** The Discover browse feed's OWN quote-asset address — falls back to `LP_GATEWAY_USDG` when unset, so a single var still works once a real rig's on-chain quote asset IS the real mainnet USDG. Must be the **real mainnet USDG** (`0x5fc5360D0400a0Fd4f2af552ADD042D716F1d168`) because `LP_GATEWAY_GT_NETWORK` reads real MAINNET GeckoTerminal data even while the deployed rig is testnet — set this explicitly whenever registry's `LP_GATEWAY_USDG` points at a testnet mock, or the feed goes empty (`usdgConfigured` reflects THIS var, falling back). |
...
| `DECK_PASSWORD` | Server-only | Password for the private investor decks (`lib/deck/gate.ts`). Gates BOTH `/deck` (full deck) and `/angels` (plain-English "Angels" edition) — one password, one cookie (`mw_deck`); unlocking either unlocks both. `POST /api/deck/unlock` validates it and sets an http-only cookie holding a hash-derived token; each gated server component renders its deck only when the cookie matches. **Unset ⇒ both gates closed to everyone (fail-closed).** Set on Vercel + `.env.local` to open them; share the value with investors out-of-band. |
| `DATAROOM_PASSWORD` | Server-only | Password for the private investor **data room** at `/dataroom` (`lib/deck/gate.ts`). SEPARATE gate from the decks — its own password and its own cookie (`mw_dataroom`), so it can be shared independently of `/deck`+`/angels`. `POST /api/dataroom/unlock` validates it and sets the http-only cookie; the `/dataroom` server component renders the data-room HTML only when the cookie matches. **Unset ⇒ gate closed to everyone (fail-closed).** Set on Vercel + `.env.local` to open it; share the value with investors out-of-band. |
...
money-moving knobs default **OFF / fail-closed**. `LP_GATEWAY_USDG` + `LP_GATEWAY_DISCOVER_USDG` (quote
asset, now two separate vars — see the 2026-09-09 note on each) are in the main table above.
...
| `LP_GATEWAY_USDG` | `registry.ts` | **set (repointed to rig 'g's tUSDG on 2026-09-08)** | The registration trust root's expected quote asset — a candidate PM's on-chain `quoteAsset()` must equal this. ⚠ **2026-09-09: discovery.ts no longer reads this directly — see `LP_GATEWAY_DISCOVER_USDG` below.** |
| `LP_GATEWAY_DISCOVER_USDG` | `discovery.ts`, curator-queue cron | **NEW 2026-09-09 — falls back to `LP_GATEWAY_USDG` if unset** | USDG address the Discover feed / curator queue match **by ADDRESS**, independent of whatever `LP_GATEWAY_USDG` is set to for registry. **Unset (and `LP_GATEWAY_USDG` also unset/wrong) ⇒ quote asset UNKNOWN or mismatched ⇒ every pool ineligible ⇒ Discover feed EMPTY + curator queue ingests nothing (fail-closed, O-7; never matched by pair name any more).** Route response carries `usdgConfigured:false` in that case. **Set this explicitly to the real mainnet USDG** (`0x5fc5360D0400a0Fd4f2af552ADD042D716F1d168`) — the fallback to `LP_GATEWAY_USDG` only helps when that var happens to ALSO be the real mainnet address, which it is not while the deployed rig runs a testnet mock quote asset. |
| `LP_GATEWAY_FACTORY` | `registry.ts` | — | The curated on-chain factory (`instanceForPool(poolId)`); when set it is THE registration trust root, checked before `LP_GATEWAY_PM_CODEHASHES`. |
| `LP_GATEWAY_PM_CODEHASHES` | `registry.ts` | **set (rig 'g' code hash, 2026-09-08)** | Comma-separated allowlist of `keccak256(runtime code)` for audited PM builds deployed directly (not via the factory) — the trust root a direct deploy uses when `LP_GATEWAY_FACTORY` is unset. Neither configured ⇒ `registerInstance` refuses everything (fail-closed). |
| `LP_GATEWAY_OWNER` | `registry.ts` | falls back to `GATEWAY_ORACLE_PRIVY_ADDRESS` | **Round-3 F-5 seat check.** The only address a registered PM's `owner()` may equal — closes the code-hash-allowlist gap where a bytecode-identical PM with an attacker owner used to pass registration on immutables alone. Unset (and no Privy address either) ⇒ `owner_env_unset`, registration fails closed. |
| `LP_GATEWAY_HARVEST_RECIPIENTS` | `registry.ts` | falls back to `{LP_GATEWAY_OWNER}` only | **Round-3 F-5 seat check.** Comma-separated allowlist a registered PM's `harvestRecipient()` must be one of, in addition to the owner — lets fees route to a separate cold address without failing the seat check. |
| `LP_GATEWAY_CURATORS` | `curateAuth.ts`, `app/api/gateway/curate` | — | **Doc drift fixed 2026-09-10 (found while verifying prod config for item 3 release-readiness — this var was entirely undocumented despite this table's own "one home" claim).** The PRIMARY curator auth as of the O-3/R-2/HO-7 closeout: comma-separated allowlist of curator wallet addresses; every approve/reject/deactivate/register is an EIP-191 signature over a canonical action-bound message the route rebuilds + strict-compares (never a shared bearer). Empty/unset allowlist ⇒ nobody is a curator (`isAllowlistedCurator` — fail-closed). **Set on prod.** |
| `LP_GATEWAY_CURATOR_SECRET` | `app/api/gateway/curate` | — | **Corrected 2026-09-10 — this row previously described a stale pre-O-3 model.** Optional server-to-server automation path ONLY (approve/reject; can NOT register an instance or deactivate one — those need a curator wallet signature via `LP_GATEWAY_CURATORS` above). Unset ⇒ that bearer path 503s `curator_secret_unset` in every environment (no `ALLOW_DEV_BEARER_BYPASS` pass-through) — the primary wallet-signature curate flow is unaffected either way. **Unset on prod today** (no server-to-server automation wired; curation is done via the signed-wallet path). |
...
| `LP_GATEWAY_HARVEST_DESTINATION` | `opsConfig.ts` (superseded — see `harvest.ts`) | ignored | **Earn-vs-LP decision (2026-09-08): dead.** `resolveHarvestDestination()` (`harvest.ts`, the actual money-path resolver) ALWAYS returns `'restake'` now regardless of this var — the A-4 per-depositor buffer ledger is dropped; LP-Gateway V1 never grows a protocol-custodied buffer. Setting this to `'buffer'` does nothing. |
...
| ~~`LP_GATEWAY_DEPLOY_RATIO_BPS`~~ | — | — | **Retired (earn-vs-lp decision, 2026-09-08).** There is no held-back buffer any more — the cron deploys the ENTIRE staged balance above the threshold every run (`MAX_DEPLOY_BPS = 10000` on-chain). Setting this var does nothing; `deploy.ts` no longer reads it. |
| `LP_GATEWAY_DEPLOY_MIN_LIQUIDITY` | `deploy.ts` | `0` | **O-9 CLOSED (round-2):** the real floor is `computeDeployMinLiquidity()`, computed PER POOL from the live spot (`getLiquidityForAmounts` haircut by `LP_GATEWAY_DEPLOY_TOL_BPS`, default 1%) — this env var is now only an OPTIONAL EXTRA floor (`max(computed, env)`). Unset/`0` does NOT block deploys; the cron still refuses only when the computed floor itself comes out 0 (`min_liquidity_unset`) or spot is out of range (`out_of_range`). |
...
| `LP_GATEWAY_ROUTER_ADDRESS` / `LP_GATEWAY_QUOTER` | `routerSwap.ts`, `v4SwapExec.ts` | — | The paired→quote swap executor + quoter, used by **harvest only** now (converting harvested paired-token fees back to quote). **Unset ⇒ that seam is a fail-closed no-op** (harvest leaves paired fees unconverted). Also needs `NEXT_PUBLIC_MW_ROUTER_ENABLED=true`. **Earn-vs-LP decision (2026-09-08):** the deploy-side use of this pair (`swapQuoteToPaired`, acquiring the paired leg off-chain) is GONE — `deploy()` executes that swap in-contract now. |
| `LP_GATEWAY_RECONCILE_ENABLED` | `reconcileSwaps.ts` | OFF | **NEW 2026-09-10** (user directive: "built it" — closes the last real V1-07 gap). `'true'` lets the reconciliation cron re-check `harvest_events` rows whose swap proceeds couldn't be measured at harvest time and, when real proceeds ARE now measurable, submit a genuine `compoundQuote()` to recover them into NAV. Unset ⇒ the cron no-ops (`ran:false, reason:'disabled'`) — rows stay flagged, nothing is lost, just unrecovered. Needs the `gateway` oracle signer seat (the owner address to check Transfer logs against comes from it — no signer this run ⇒ the WHOLE pass skips, never guesses an owner). ⚠ Not scheduled in `vercel.json` — bearer route, run by hand, same as harvest/deploy. |
| `LP_GATEWAY_RECONCILE_BATCH_SIZE` | `reconcileSwaps.ts` | `25` | Max pending-reconciliation rows checked per cron invocation. Clamped 1–200; malformed falls back to the default. |
| `LP_GATEWAY_SWAP_SLIPPAGE_BPS` | `deploy.ts` (moved off `v4SwapExec.ts` — earn-vs-lp decision) | `100` (1%) | Slippage floor for `deploy()`'s in-contract zap (`minPairedOut`, via `quoteToPairedAtSpot` + this haircut). Same knob, now gating the on-chain swap instead of an off-chain executor. |
...
| `LP_GATEWAY_IDLE_MODE` | `preflight-lp-gateway-mainnet.mjs`, `deploy-lp-gateway-mainnet.mjs` (deploy-time) | OFF | `'true'` opt-in to the zero-yield `MintwareIdleYieldAdapter` (`contracts-v4/src/vaults/MintwareIdleYieldAdapter.sol`) when no real ERC-4626 USDG vault has capacity — see [`lp-gateway.md`](lp-gateway.md)'s mainnet-yield-source note. Requires `LP_GATEWAY_DEPOSIT_CAP`. Setting a real `LP_GATEWAY_YIELD_SOURCE` at the same time is a preflight **FAIL** (contradictory intents — IA-13) unless acknowledged via `LP_GATEWAY_IDLE_MODE_IGNORES_SOURCE=true`. |
| `LP_GATEWAY_DEPOSIT_CAP` | `MintwareIdleYieldAdapter` ctor (deploy-time, idle mode only) | — | Adapter-level cap on cumulative supplied principal (`suppliedPrincipal`, not raw balance — donation-griefing-proof, IA-3). Required in idle mode, even `0` (closed) is valid; **only bounds the idle leg** — see `LP_GATEWAY_PRINCIPAL_CAP` for the real total-value-at-risk bound (IA-11). |
| `LP_GATEWAY_PRINCIPAL_CAP` | `MintwareLpGatewayPositionManager` ctor (deploy-time, both paths) | idle mode: defaults to `LP_GATEWAY_DEPOSIT_CAP`; real-adapter path: **required, no default** | **IA-11 fix.** The on-chain `principalCap` bounding `idle + deployedPrincipal + deployedPairedValue` — the genuine ceiling on total depositor-relevant value. `deposit`/`deploy` revert `PrincipalCapExceeded` above it; owner-adjustable post-deploy via `setPrincipalCap`. **Earn-vs-LP decision (2026-09-08):** `deploy()` no longer accepts an owner-supplied paired leg at all (deleted outright) — `deployedPairedValue` is now 100% USER value that changed form via the in-contract zap, so this cap is simpler than when it was written (nothing external left to bound; it just caps total deposits). Preflight also asserts both this and `LP_GATEWAY_DEPOSIT_CAP` are ≤ a 100,000 USDG ceiling unless `LP_GATEWAY_ALLOW_LARGE_CAP=true` (IA-12). |
