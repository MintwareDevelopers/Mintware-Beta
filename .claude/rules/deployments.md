...
| `LP_GATEWAY_USDG` | `discovery.ts`, `meta` route, registry | **set (repointed to rig 'g's tUSDG on 2026-09-08)** | USDG address the Discover feed / curator queue / registry match **by ADDRESS**. **Unset ⇒ quote asset UNKNOWN ⇒ every pool ineligible ⇒ Discover feed EMPTY + curator queue ingests nothing (fail-closed, O-7; never matched by pair name any more).** Route response carries `usdgConfigured:false` while unset. |
