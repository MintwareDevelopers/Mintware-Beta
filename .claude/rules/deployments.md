...
| `X402_SETTLE_PROVIDER` | Server-only | `oracle` → settle **in-process via `getOracleSigner('root')`** (the SAME Privy/oracle signer the card flow uses — no separate relayer service, no raw key; honors `ORACLE_SIGNER_PROVIDER=privy`). This is the **platform-consistent** settle path. Unset (default) → `deferredSettler` unless `X402_RELAYER_URL` is set. Precedence: `X402_RELAYER_URL` (Rust relayer) > `X402_SETTLE_PROVIDER=oracle` > deferred (`lib/x402/config.ts#getSettler`). Needs `X402_GATEWAY_ADDRESS` + `X402_PERMIT_CHAIN_ID`; the Privy signer must hold `RELAYER_ROLE` on the gateway. |
