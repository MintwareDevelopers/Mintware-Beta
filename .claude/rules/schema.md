...
- `20260805000001_kyc_records.sql` — **WS1 RWA trader gate.** `kyc_records` (address pk, status/level, `provider_hash` = keccak(inquiryId), `onchain_status`/`onchain_tx`), RLS-on/no-policies (service-role only). Mirror of the on-chain `SPVBeneficiaryRegistry`; written by `/api/kyc/webhook`, read by `/api/kyc/status`. **Created, NOT yet applied to prod Supabase.**
