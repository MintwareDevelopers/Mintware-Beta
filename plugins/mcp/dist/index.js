...
function formatScore(profile, score, mwpHashes) {
        ...
        tools: [
            {
                ...
                description: "Fetch the Mintware AI Attribution score for an agent wallet address on Base mainnet. " +
            ...
            },
            {
                ...
                description: "Fetch the top AI agents on the Mintware Attribution leaderboard on Base mainnet. " +
            ...
            },
            {
                ...
                description: "Register an AI agent wallet with the Mintware AI Attribution contract on Base mainnet. " +
                ...
                inputSchema: {
                    ...
                    properties: {
                        privateKey: {
                            type: "string",
                            description: "Private key of the agent wallet to register (0x-prefixed hex, 66 characters). " +
            ...
            },
            {
                ...
                description: "Fetch pending oracle-signed action attestations from the Mintware API and submit them " +
                ...
                inputSchema: {
                    ...
                    properties: {
                        ...
                        privateKey: {
                            type: "string",
                            description: "Private key of the agent wallet (0x-prefixed hex, 66 characters). " +
...
server.setRequestHandler(types_js_1.CallToolRequestSchema, async (request) => {
    ...
    if (name === "mintware_leaderboard") {
        ...
        try {
            const res = await fetch(`${API_BASE}/api/agents/leaderboard?limit=${clampedLimit}`);
            if (!res.ok) {
                throw new Error(`API returned ${res.status}: ${res.statusText}`);
            }
            const data = (await res.json());
    ...
    if (name === "mintware_register") {
        ...
        try {
            const { address, txHash } = await (0, ai_attribution_sdk_1.registerWithMintwareOracle)({
    ...
    if (name === "mintware_claim_pending") {
        ...
        try {
            const result = await (0, ai_attribution_sdk_1.claimPendingActions)({
                agent: address,
                privateKey: privateKey,
                contractAddress: BASE_MAINNET_CONTRACT,
                apiBase: API_BASE,
