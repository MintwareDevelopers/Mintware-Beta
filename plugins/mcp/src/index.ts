  ...
  return {
    tools: [
      ...
      },
      {
        ...
        description:
          "Get an executable swap quote via Mintware's LI.FI proxy. Returns LI.FI's raw quote INCLUDING the " +
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (name === "mintware_swap_quote") {
    const { chainId, sellToken, buyToken, sellAmount, taker } = (args ?? {}) as {
    ...
    try {
      ...
      return {
        content: [{
          type: "text",
          text:
            "Executable swap quote (LI.FI). The `transactionRequest` below is the tx to sign and broadcast " +
