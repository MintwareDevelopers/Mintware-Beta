...
const mintwareSwapExecuteAction = {
    ...
    description: 'Execute a same-chain token swap FROM THE AGENT WALLET: fetches an executable quote via Mintware\'s ' +
    ...
    schema: z.object({
        chainId: z.number().describe('EVM chain id (e.g. 8453 for Base).'),
        sellToken: z.string().describe('Token address being sold (0x…). Use the native sentinel 0xEeee…EEeE or 0x0000…0000 for the chain\'s native coin.'),
        buyToken: z.string().describe('Token address being bought (0x…).'),
        sellAmount: z.string().describe('Amount to sell, in raw token units (atomic/wei).'),
        minBuyAmount: z.string().optional().describe('Optional slippage guard (raw units): abort if the quote\'s guaranteed minimum output is below this.'),
    }),
    invoke: async (wallet, args) => {
        if (!wallet.sendTransaction) {
